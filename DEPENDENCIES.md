...
| flutter_map | 7.0.2 | BSD-3-Clause | https://pub.dev/packages/flutter_map | Phase 09.1 plan 09.1-01 — same-canvas fog engine (POC `mirk-poc-debug` @ 90c9321, VERDICT.md PORT BACK, validated on iPhone 17 Pro + Pixel 4a), replaces `maplibre_gl 0.25.0` (cf. BUG-014 — fog must live on the same canvas as the map). Coexists with maplibre_gl for one wave; maplibre_gl is removed in plan 09.1-02 Task 2. POC audit 2026-05-05 re-run 2026-09-09. LICENSE file in pub cache: `BSD 3-Clause License, Copyright (c) 2018-2024, the 'flutter_map' authors and maintainers` (matches pub.dev). Network audit (`grep -rn "package:http\|HttpClient\|dart:io" lib/`): `package:http` imported only by `src/layer/tile_layer/tile_layer.dart` (default RetryClient for the raster NetworkTileProvider), `tile_provider/network_tile_provider.dart`, `tile_provider/network_image_provider.dart`; `dart:io` only in `tile_provider/file_providers/tile_provider_io.dart`. MirkFall never instantiates `NetworkTileProvider` / raster `TileLayer` — tiles come from `vector_map_tiles_pmtiles` over local PMTiles archives; `check_avoid_remote_pmtiles` + `check_style_no_external_url` stay in force. Telemetry grep (firebase / crashlytics / sentry / mixpanel / amplitude / appsflyer / adjust / analytics): 0 matches. Pure Dart, no native platform dirs → iOS + Android + Windows desktop (bonus: the map now renders under `flutter run -d windows`). Maintenance: 7.x line frozen ~22 months, 8.x active upstream (fleaflet org) — pinned to 7.0.2 deliberately for exact parity with the validated POC code. Import perimeter enforced by `tool/check_avoid_flutter_map_leak.dart` (MAP-06). | 2026-09-09 |
...
| pmtiles | 1.2.0 | BSD-2-Clause | https://pub.dev/packages/pmtiles | Phase 09.1 plan 09.1-01 — PMTiles v3 archive reader (root/leaf directory lookup, gzip decompression) feeding `vector_map_tiles_pmtiles`. LICENSE file: `BSD 2-Clause License, Copyright (c) 2023-2024, Andrew Brampton` (matches pub.dev). Network audit (`grep -rn "package:http\|HttpClient\|dart:io" lib/`): `package:http` in `src/io.dart` (`HttpAt` HTTP-Range reader) + `src/archive.dart` (`PmTilesArchive.fromUri` selects HttpAt for http(s) URIs, FileAt otherwise); MirkFall opens archives via `fromFile` / `FileAt` only, so `HttpAt` is never instantiated — `check_avoid_remote_pmtiles` guards the URI scheme at lint time. `dart:io` for `FileAt` (RandomAccessFile). Telemetry grep: 0. Pure Dart → iOS + Android + Windows. Maintenance: 2.x available; 1.2.0 pinned — the version resolved by vector_map_tiles_pmtiles 1.5.0 and validated in the POC (bramp). | 2026-09-09 |
...
| vector_map_tiles | 8.0.0 | BSD-3-Clause | https://pub.dev/packages/vector_map_tiles | Phase 09.1 plan 09.1-01 — `VectorTileLayer` for flutter_map: fetches MVT tiles from a `VectorTileProvider`, decodes off-thread via `executor_lib`, paints through `vector_tile_renderer`. Replaces the MapLibre native renderer for the Protomaps PMTiles catalog (MAP-05..08 pipeline unchanged). LICENSE file: `Copyright 2021 David Green` BSD-3-Clause (matches pub.dev). Network audit (`grep -rn "package:http\|HttpClient\|dart:io" lib/`): `package:http` only in `src/provider/network_vector_tile_provider.dart` (`NetworkVectorTileProvider`) + `src/style/style.dart` (`StyleReader` for remote style JSON) — neither instantiated by MirkFall (provider = `PmTilesVectorTileProvider`, theme parsed from the bundled `assets/maps/style.json`). `dart:io` in `src/cache/*` (file tile cache under `getTemporaryDirectory()`), `options.dart`, `vector_tile_layer.dart`, `grid/tile_model.dart` — local file I/O only. Telemetry grep: 0. Pure Dart → iOS + Android + Windows. Maintenance: 8.0 stable ~20 months; 9.x in beta — pinned 8.0.0 for POC parity (greensopinion). Source-key contract: `TileProviders` key MUST equal the style `sources` key (`kMapStyleSourceKey`), debug-only assert upstream. | 2026-09-09 |
| vector_map_tiles_pmtiles | 1.5.0 | MIT | https://pub.dev/packages/vector_map_tiles_pmtiles | Phase 09.1 plan 09.1-01 — `PmTilesVectorTileProvider` bridging `pmtiles` archives into `vector_map_tiles` (world bundle + per-country hot-swap, MAP-01 zero-network tiles). LICENSE file: `MIT License, Copyright (c) 2024 Joscha Eckert (josxha)` (matches pub.dev). Network audit (`grep -rn "package:http\|HttpClient\|dart:io" lib/`): 0 matches — all archive I/O delegated to `pmtiles`. Telemetry grep: 0. Pure Dart → iOS + Android + Windows. Maintenance: 1.5 stable ~18 months (josxha/flutter_map_plugins monorepo, active). | 2026-09-09 |
| vector_tile_renderer | 5.2.0 | BSD-3-Clause | https://pub.dev/packages/vector_tile_renderer | Phase 09.1 plan 09.1-01 — pure-Dart MVT decoder + Canvas renderer + Mapbox/MapLibre style-spec interpreter (`Theme`) used by `vector_map_tiles`; consumes `assets/maps/style.json` verbatim (frozen 7-layer order, `mirk_fog` placeholder layer ignored). LICENSE file: `Copyright 2021 David Green` BSD-3-Clause (matches pub.dev). Network audit (`grep -rn "package:http\|HttpClient\|dart:io" lib/`): 0 matches. Telemetry grep: 0. Pure Dart → iOS + Android + Windows. Maintenance: 5.x line active; 6.x available but outside the vector_map_tiles 8.0.0 constraint — pinned 5.2.0 for POC parity (greensopinion). | 2026-09-09 |
...
| dart_earcut | 1.2.0 | MIT | flutter_map | Phase 09.1 (2026-09-09). Pure-Dart port of mapbox/earcut — polygon triangulation for flutter_map's `PolygonLayer` (not used by MirkFall; reveal discs are painted by the fog layer). LICENSE file: `MIT License, Copyright (c) 2025 Luka Stillingfleet (JaffaKetchup)`. Zero network. | 2026-09-09 |
...
| executor_lib | 1.1.2 | BSD-3-Clause | vector_map_tiles | Phase 09.1 (2026-09-09). Isolate-pool executor used by vector_map_tiles for off-main-thread MVT tile decoding. LICENSE file: `Copyright 2021 David Green` BSD-3-Clause. Zero network. | 2026-09-09 |
...
| intl | 0.20.3 | BSD-3-Clause | latlong2 | Phase 09.1 (2026-09-09). Dart-team i18n / number formatting (latlong2 uses it for coordinate formatting). LICENSE file: `Copyright 2013, the Dart project authors` BSD-3-Clause. `dart:io` only in `intl_standalone.dart` (system-locale lookup) + `src/file_data_reader.dart` (local file reads) — no network. Resolved 0.20.3 (POC lock carried 0.20.2 — newer patch inside latlong2's `^0.20.0` constraint, no other consumer). | 2026-09-09 |
...
| mgrs_dart | 2.0.0 | MIT | proj4dart | Phase 09.1 (2026-09-09). MGRS ↔ lat/lon conversion used by proj4dart. LICENSE file: `MIT License, Copyright (c) 2020 Gergely Padányi-Gulyás`. Zero network. | 2026-09-09 |
...
| unicode | 0.3.1 | BSD-3-Clause | mgrs_dart | Phase 09.1 (2026-09-09). Unicode character-class tables. LICENSE file: `Copyright (c) 2014, Andrew Mezoni` BSD-3-Clause. Zero network. | 2026-09-09 |
...
| vector_tile | 2.0.1 | MIT | vector_tile_renderer | Phase 09.1 (2026-09-09). Mapbox Vector Tile (MVT) protobuf schema + decoder. LICENSE file: `The MIT License (MIT), Copyright (c) 2021 Saigontek`. Zero network. | 2026-09-09 |
...
| wkt_parser | 2.0.0 | MIT | proj4dart | Phase 09.1 (2026-09-09). Well-Known-Text CRS definition parser. LICENSE file: `MIT License, Copyright (c) 2020 Gergely Padányi-Gulyás`. Zero network. | 2026-09-09 |
...
|--------------|----------------|---------|-------------|------|
| Protomaps basemaps style (neutral variant) | github.com/protomaps/basemaps (style skeleton repo) | BSD-3-Clause (software) + CC0-1.0 (map design) | Phase 07 plan 07-01 — the JSON style ships as `assets/maps/style.json` (frozen 6-layer order since Phase 09.1: `mirk_fog` removed, no `glyphs` / `sprite` keys — text via `vector_tile_renderer` + platform font). Software = style JSON schema authoring; the cartographic *design* (colours, widths, z-order choices) is CC0-1.0 per Protomaps' stated policy. Attribution carried alongside OSM attribution. | 2026-04-20 |
