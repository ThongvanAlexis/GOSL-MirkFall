...
- **Téléchargement en arrière-plan (écran verrouillé / app backgroundée)** → **V2 Backlog (voir `PROJECT.md §V2 Backlog`)**. Amendement post-device-smoke 2026-04-21 : l'utilisateur a verrouillé le téléphone pendant un download et le transfert s'est mis en pause. Cause racine : l'isolate Flutter est suspendu par l'OS (Android Doze + iOS suspension) quand l'app backgroundée n'a pas de foreground service. V1.0 se contente du resume Range-based au retour au foreground — l'infrastructure pipeline (sha256 par chunk, atomic rename, manifest tempfile+rename) la survit correctement. V2 livre la vraie expérience continue : Android Foreground Service dédié + iOS `URLSession.backgroundConfiguration` via platform channel. Contrepartie iOS : perte du pur-Dart `HttpChunkDownloader` testable pour la partie iOS. Détails + justification dans `PROJECT.md §V2 Backlog`.
