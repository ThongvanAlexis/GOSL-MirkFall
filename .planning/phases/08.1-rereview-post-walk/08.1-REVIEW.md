...
| 2 | A1#1 | Pause-then-Cancel leaves state stranded as DownloadPaused — `pmtiles_download_controller.dart:238-243,327-332` | Should | `[smell:fix-on-fix]` | fix | Should — concrete UX bug; add cancel-in-paused branch + test | `b1e407a` |
| 3 | A2#2 | MapCamera echo-window mis-classifies second echo as user pan — `map_camera_controller.dart:325-332` | Should | `[smell:fix-on-fix]` | fix | Should — follow-me regression under multi-echo; keep timestamp alive for full window | `1fda0f2` |
| 4 | A2#3 | MapCamera `toggleFollowMe` race between `_moveCameraTo` await + state assign — `map_camera_controller.dart:202-213` | Should | — | fix | Should — reorder state assignment before the await OR flip to Following(hasFirstFix:false) first | `e338787` |
| 5 | A2#4 | CountryResolver first-listen race (write dropped between `repo.read()` + `repo.updates.listen`) — `country_resolver_controller.dart:198-212` | Should | — | fix | Should — attach listener first THEN read, or buffer early writes | `5505101` |
| 6 | A2#5 | `_rebuildResolver` reads catalog synchronously — cold-start no banner for non-installed country — `country_resolver_controller.dart:223-230` | Should | — | fix | Should — watch catalogProvider + rebuild on AsyncData | `5505101` |
| 7 | A1#2 | `_alpha3IsActiveOrQueued` duplicates row #20 polymorphic dispatch — `pmtiles_download_controller.dart:212-221` | Could | `[smell:over-state-machine]` | fix | Could — refactor to `_state.activeJob?.alpha3 == alpha3` one-liner | `25d963d` |
| 8 | A1#3 | Row #8 heal + purge writes manifest twice per run — `first_launch_bootstrap.dart:129-134,222-224,265-267` | Could | — | fix | Could — consolidate into single write at end of `run()` | `33e9292` |
| 9 | A2#6 | `_bestEffort` logs via `_log.fine` (below default INFO threshold) — `active_session_controller.dart:199-205` | Could | — | fix | Could — flip to `_log.warning` for consistency + CLAUDE.md §Error handling compliance | `570084b` |
| 10 | A2#7 | `MapCameraFollowing.isCentering` getter re-discriminates sealed variant — `map_camera_controller.dart:62` | Could | — | fix | Could — revisit pattern-match at FAB (finding cross-refs A3) | `55ac7ce` |
| 11 | A2#8 | `_tearDown` via `ref.onDispose` accumulates across rebuild — `map_camera_controller.dart:127-149` | Could | — | fix | Could — ensure single subscription guard on provider invalidate | `f459f44` |
| 12 | A3#1 | Row #43 duration-hoist incomplete — 2 magic Durations remain in `_confirmAndEnqueue` — `maps_download_screen.dart:362,367` | Could | — | fix | Could — hoist both literals into named constants | `f05e73c` |
| 13 | A3#4 | MapScreen.deactivate rotation null-gap — brief follow-me loss if GPS fix lands — `map_screen.dart:68-122` | Could | `[smell:fix-on-fix]` | fix | Could — guard or debounce the `_nullifyMapViewProviderAfterDeactivate` path on re-parent | `4fc1f52` |
| 14 | A4#1 | Stale "frozen 8-layer" docstring in pubspec.yaml (live sources 7-layer) — `pubspec.yaml:183` | Could | — | fix | Could — one-line doc update | `20910f8` |
| 15 | A4#2 | ROADMAP Phase 08.1 plan checkboxes not synced to Progress row — `ROADMAP.md:188-189` | Could | — | fix | Could — flip 08.1-01 + 08.1-02 to `[x]` | `bb6c78e` |
...
- **Final commit on main:** `1c3400c` — `chore(08.1-rev): dart format align maps_download_screen.dart after row #12 hoist`
- **CI run URL:** https://github.com/ThongvanAlexis/GOSL-MirkFall/actions/runs/24900922812
- **Status:** All 3 jobs green (gates / android / ios) — run 24900922812 on final commit `1c3400c`
- **Date:** 2026-04-24

**Fix-loop summary** (Strategy A per-finding atomic commits, 08.1-RESEARCH §Open Question 3):

- 15 §3 `fix` decisions landed as 14 atomic commits (rows #5 + #6 merged — both CountryResolver async-boot race closures in adjacent listener-wiring code).
- 1 follow-up `chore(08.1-rev)` commit — dart format align of `maps_download_screen.dart` after the row #12 Duration-hoist nudged lines past the 160-char limit.
- Commit-prefix split (15 total): 8 × `fix` (rows #1 #2 #3 #4 #5+#6 #9 #12 #13), 3 × `refactor` (smell-tagged rows #7 #8 #10), 3 × `docs` (row #11 docstring + row #14 pubspec text + row #15 ROADMAP checkboxes), 1 × `chore` (format align follow-up).
- All 8 CI gate scripts re-run locally on final commit: green (300 headers / 189 licenses / 189 deps / 57 domain / platform manifests / 149 maplibre-leak / 536 remote-pmtiles / style-no-external-url).
- Full test suite on final commit: 731 / 731 passed (excluding soak + integration tags per the project runner default).
- Zero new adversarial branches (default-skip convention held 5th cycle, locked Phases 02+04+06+08+08.1).

---
_Phase 08.1 closed: 2026-04-24_
