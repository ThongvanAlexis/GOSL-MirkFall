...
void main() {
  group('09-03 — computeRevealMask (MIRK-01)', () {
    test('circle fully inside parent tile produces a disc-shaped bitmap', () {
      ...
      expect(mask, isA<Uint8List>(), reason: 'mask must be the canonical Uint8List buffer');
      expect(mask.length, kRevealedTileBitmapBytes, reason: 'mask must be 512 bytes (64×64 bits)');
      ...
      expect(setBits, greaterThanOrEqualTo(3), reason: 'expected ≥ 3 cells revealed for a 25 m disc at lat 45°');
      expect(setBits, lessThanOrEqualTo(12), reason: 'expected ≤ 12 cells revealed for a 25 m disc at lat 45°');
    });
    ...
    test('circle fully outside parent tile returns an all-zero mask', () {
      ...
      const fixLat = 45.0;
      const fixLon = 5.0;
      final farTile = TileMath.latLonToTile(lat: 45.0, lon: 100.0, zoom: kRevealedTileParentZoom);
      final mask = computeRevealMask(
        centerLat: fixLat,
        centerLon: fixLon,
        radiusMeters: kDefaultRevealRadiusMeters,
      ...
      expect(popcount(mask), 0, reason: 'circle outside parent tile must produce a no-op mask');
    });
    ...
    test('tiny radius (1 m) flips at most 4 cells', () {
      ...
      final setBits = popcount(mask);
      expect(setBits, greaterThanOrEqualTo(1), reason: '1 m disc must still flip at least the home cell');
      expect(setBits, lessThanOrEqualTo(4), reason: '1 m disc cannot intersect more than 4 cells');
    });
    ...
    test('large radius (120 m) covers inner cells far from the circle edge', () {
      ...
      final setBits = popcount(mask);
      expect(setBits, greaterThanOrEqualTo(30), reason: '120 m disc must reveal a substantial inner area (>=30 cells)');
      ...
      expect(mask[centreByteIndex] & (1 << centreBitOffset), isNonZero, reason: 'centre cell must be revealed by a 120 m disc');
    });

    test('polar latitude (lat=85) does not crash and yields a valid 512-byte mask', () {
    ...
    test('MIRK-03 no-micro-hole: cell partially touched by circle edge still flips', () {
      ...
      expect(popcount(mask), greaterThan(0), reason: 'MIRK-03 invariant: edge-touched cell must flip even when its centre is outside the circle');
    });
  });
  ...
  group('09-03 — computeRevealMask Phase 03 contract regression', () {
    ...
    test('returns a 512-byte Uint8List for well-formed input (no longer throws)', () {
      final mask = computeRevealMask(centerLat: 0.0, centerLon: 0.0, radiusMeters: 50.0, parentX: 0, parentY: 0, parentZoom: kRevealedTileParentZoom);
