...
void main() {
  ...
  test('precondition: the seed has 1 session + 2 markers + 3 tiles + 1 photo',
      () async {
    expect(
      await _count(db, 't_sessions', 'id = ?', <Object?>[sessionId.value]),
      1,
    );
    expect(
      await _count(
          db, 't_markers', 'session_id = ?', <Object?>[sessionId.value]),
      2,
    );
    expect(
      await _count(db, 't_revealed_tiles', 'session_id = ?',
  ...
  test('DriftSessionStore.delete cascades to markers, revealed_tiles '
    ...
    expect(
      await _count(
          db, 't_markers', 'session_id = ?', <Object?>[sessionId.value]),
      0,
      reason: 'markers cascaded via FK ON DELETE CASCADE',
    );
    expect(
      await _count(db, 't_revealed_tiles', 'session_id = ?',
          <Object?>[sessionId.value]),
      0,
      reason: 'revealed tiles cascaded via FK ON DELETE CASCADE',
    );
    expect(
      await _count(db, 't_photos', 'marker_id = ?', <Object?>['mrk_C1']),
      0,
      reason: 'photos cascaded through the marker delete '
    ...
    expect(
      await _count(
          db, 't_marker_categories', 'id = ?', <Object?>['cat_default']),
      1,
      reason: 'cat_default is not cascaded (non-CASCADE policy)',
    );
  });
}
