...
void main() {
  ...
  test('MIRK-03: t_revealed_tiles composite unique key enforces idempotence',
      () async {
    await db.customStatement(
      "INSERT INTO t_sessions (id, display_name, status, started_at_utc, "
  ...
  test('CASCADE: deleting a session removes its markers + revealed_tiles',
      () async {
  ...
  test('t_sessions.notes column exists (V2 shape)', () async {
    ...
    final notesCol = rows
        ...
        .toList();
    expect(notesCol, hasLength(1),
        reason: 'V2 schema must include the notes column');
    expect(notesCol.single.read<int>('notnull'), 0,
