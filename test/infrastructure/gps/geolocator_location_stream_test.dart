...
void main() {
  ...
  test('lastKnownFix is null before any emission', () async {
    ...
    final sub = stream.positions(sessionId: sessionId, distanceFilterMeters: 5, sessionDisplayName: 'Test').listen((_) {});
  ...
  test('lastKnownFix is NOT updated on rejected emissions (accuracy filter)', () async {
    final sub = stream.positions(sessionId: sessionId, distanceFilterMeters: 5, sessionDisplayName: 'Test').listen((_) {});
    ...
    positionController.add(_position(latitude: 99.0, longitude: 99.0, accuracyMeters: 60.0));
    await Future<void>.delayed(Duration.zero);
    expect(stream.lastKnownFix, same(firstFix), reason: 'rejected position must not overwrite lastKnownFix');

    await sub.cancel();
  });

  test('lastKnownFix survives dispose (cache persists for short-reconnect)', () async {
    final sub = stream.positions(sessionId: sessionId, distanceFilterMeters: 5, sessionDisplayName: 'Test').listen((_) {});
