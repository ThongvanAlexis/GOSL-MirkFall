...
void main() {
  group('09-04 — MirkProjection.latLonToScreen', () {
    ...
    test('NW corner (lat=north, lon=west) maps to (0, 0)', () {
      final off = MirkProjection.latLonToScreen(lat: 44.0, lon: 5.0, viewport: bbox, size: size);
      expect(off.dx, closeTo(0.0, 1e-9));
      expect(off.dy, closeTo(0.0, 1e-9));
    });

    test('SE corner (lat=south, lon=east) maps to (size.width, size.height)', () {
      final off = MirkProjection.latLonToScreen(lat: 43.0, lon: 6.0, viewport: bbox, size: size);
      expect(off.dx, closeTo(400.0, 1e-9));
      expect(off.dy, closeTo(800.0, 1e-9));
    });

    test('NE corner (lat=north, lon=east) maps to (size.width, 0)', () {
      final off = MirkProjection.latLonToScreen(lat: 44.0, lon: 6.0, viewport: bbox, size: size);
      expect(off.dx, closeTo(400.0, 1e-9));
      expect(off.dy, closeTo(0.0, 1e-9));
    });

    test('SW corner (lat=south, lon=west) maps to (0, size.height)', () {
      final off = MirkProjection.latLonToScreen(lat: 43.0, lon: 5.0, viewport: bbox, size: size);
      expect(off.dx, closeTo(0.0, 1e-9));
      expect(off.dy, closeTo(800.0, 1e-9));
    });

    test('centre (lat=43.5, lon=5.5) maps to (width/2, height/2)', () {
      final off = MirkProjection.latLonToScreen(lat: 43.5, lon: 5.5, viewport: bbox, size: size);
    ...
    test('zero-span bbox (north == south) returns Offset.zero (defensive)', () {
      final zeroLatSpan = MirkViewportBbox(south: 44.0, west: 5.0, north: 44.0, east: 6.0);
      final off = MirkProjection.latLonToScreen(lat: 44.0, lon: 5.5, viewport: zeroLatSpan, size: size);
      expect(off, equals(Offset.zero));
    });

    test('zero-span bbox (east == west) returns Offset.zero (defensive)', () {
      final zeroLonSpan = MirkViewportBbox(south: 43.0, west: 5.0, north: 44.0, east: 5.0);
      final off = MirkProjection.latLonToScreen(lat: 43.5, lon: 5.0, viewport: zeroLonSpan, size: size);
      expect(off, equals(Offset.zero));
    });
  });
}
