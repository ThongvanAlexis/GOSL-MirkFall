...
class _FakeRevealedTileStore implements RevealedTileStore {
  ...
  final Map<({int parentX, int parentY}), Uint8List> _accumulated = <({int parentX, int parentY}), Uint8List>{};
  ...
  Future<void> mergeMask({required SessionId sessionId, required int parentX, required int parentY, required Uint8List mask}) async {
    ...
    if (throwOnNthCall == callIndex) {
      throw StateError('FakeRevealedTileStore forced throw on call #$callIndex');
    }
  ...
  Uint8List? readAccumulated({required int parentX, required int parentY}) => _accumulated[(parentX: parentX, parentY: parentY)];
...
void main() {
  group('09-06 — RevealStreamingController (MIRK-01)', () {
    test('count-bound flush — N fixes triggers immediate flush at threshold', () async {
      ...
      expect(store.mergeMaskCalls, isEmpty, reason: 'count-bound flush must NOT fire below threshold');

      // 3rd fix — count threshold hit, flush fires immediately.
      await controller.onFix(_buildFix(lat: 45.0002, lon: 5.0, suffix: '0C'));
      expect(store.mergeMaskCalls.length, greaterThanOrEqualTo(3), reason: 'one mergeMask call per fix at minimum (single-tile case)');
    });

    test('time-bound flush — buffered fixes flush after flushInterval (manual flush() proxy)', () async {
      ...
      final store = _FakeRevealedTileStore();
      final controller = RevealStreamingController(sessionId: _testSessionId, store: store, flushInterval: const Duration(seconds: 60), flushMaxFixes: 100);
      addTearDown(controller.dispose);

      await controller.onFix(_buildFix(lat: 45.0, lon: 5.0, suffix: '0A'));
      await controller.onFix(_buildFix(lat: 45.0, lon: 5.0001, suffix: '0B'));
      expect(store.mergeMaskCalls, isEmpty, reason: 'no flush below count threshold');

      await controller.flush();
      expect(store.mergeMaskCalls.length, greaterThanOrEqualTo(2), reason: 'manual flush() must drain buffered fixes');
    });

    test('revealInitial writes a smaller-radius mask without buffering', () async {
      final store = _FakeRevealedTileStore();
      final controller = RevealStreamingController(sessionId: _testSessionId, store: store, flushInterval: const Duration(seconds: 60), flushMaxFixes: 100);
      addTearDown(controller.dispose);

      await controller.revealInitial(_buildFix(lat: 45.0, lon: 5.0));
      expect(store.mergeMaskCalls.length, greaterThanOrEqualTo(1), reason: 'revealInitial must bypass the buffer and write immediately');
      ...
      final homeTile = TileMath.latLonToTile(lat: 45.0, lon: 5.0, zoom: kRevealedTileParentZoom);
      final initialBitmap = store.readAccumulated(parentX: homeTile.x, parentY: homeTile.y);
      ...
      final defaultBitmap = defaultStore.readAccumulated(parentX: homeTile.x, parentY: homeTile.y);
      expect(defaultBitmap, isNotNull);
      final defaultPopcount = _popcount(defaultBitmap!);

      expect(initialPopcount, lessThanOrEqualTo(defaultPopcount), reason: 'kInitialRevealRadiusMeters (20 m) covers ≤ kDefaultRevealRadiusMeters (25 m) area');
      expect(initialPopcount, greaterThan(0), reason: 'a 20 m initial reveal at a tile-interior fix must set ≥ 1 bit');
    });

    test('fix at parent-tile boundary produces ≥ 2 distinct tile writes', () async {
      final store = _FakeRevealedTileStore();
      final controller = RevealStreamingController(sessionId: _testSessionId, store: store, flushInterval: const Duration(seconds: 60), flushMaxFixes: 1);
      addTearDown(controller.dispose);
      ...
      const homeLat = 45.0;
      const homeLon = 5.0;
      final homeTile = TileMath.latLonToTile(lat: homeLat, lon: homeLon, zoom: kRevealedTileParentZoom);
      final eastTileNw = TileMath.tileToLatLon(x: homeTile.x + 1, y: homeTile.y, zoom: kRevealedTileParentZoom);
      ...
      final touchedTiles = store.mergeMaskCalls.map((c) => '${c.parentX}_${c.parentY}').toSet();
      expect(
        touchedTiles.length,
        greaterThanOrEqualTo(2),
        reason: 'a 25 m disc at a parent-tile boundary must straddle ≥ 2 tiles. Got writes for: $touchedTiles',
      );
    });
    ...
    test('dispose flushes any pending buffered fixes', () async {
      ...
      await controller.onFix(_buildFix(lat: 45.0, lon: 5.0, suffix: '0A'));
      await controller.onFix(_buildFix(lat: 45.0001, lon: 5.0, suffix: '0B'));
      expect(store.mergeMaskCalls, isEmpty, reason: 'no flush below count + interval threshold');

      await controller.dispose();
      expect(store.mergeMaskCalls.length, greaterThanOrEqualTo(2), reason: 'dispose() must flush pending fixes — no data loss on session end');
    });

    test('repeated identical fixes preserve OR-stable monotone bitmap', () async {
      final store = _FakeRevealedTileStore();
      final controller = RevealStreamingController(
        sessionId: _testSessionId,
        store: store,
      ...
      final after3 = store.readAccumulated(parentX: homeTile.x, parentY: homeTile.y);
      ...
      await controller.onFix(_buildFix(lat: fixLat, lon: fixLon, suffix: '0D'));
      final after4 = store.readAccumulated(parentX: homeTile.x, parentY: homeTile.y)!;
      expect(_popcount(after4), greaterThanOrEqualTo(beforeFourthPopcount));
      expect(_popcount(after4), beforeFourthPopcount);
    });

    test('mergeMask failure on one tile is logged and batch continues', () async {
      ...
      expect(store.mergeMaskCalls.length, greaterThanOrEqualTo(2), reason: 'second tile write must still happen after first one threw');
    });

    test('flush() while no fixes buffered is a no-op', () async {
      final store = _FakeRevealedTileStore();
      final controller = RevealStreamingController(sessionId: _testSessionId, store: store);
      addTearDown(controller.dispose);
