...
const MirkStyleId _styleAtmospheric = MirkStyleId('mst_01HRMIRKSTYLEATMOSPHEREXXXX');
const MirkStyleId _styleSolid = MirkStyleId('mst_01HRMIRKSTYLESOLIDXXXXXXXXX');
const MirkStyleId _styleUnknown = MirkStyleId('mst_01HRMIRKSTYLENONEXISTENTXXX');
...
class _FakeSessionStore implements SessionStore {
  ...
  final List<({SessionId sessionId, MirkStyleId? mirkStyleId})> updateMirkStyleCalls = <({SessionId sessionId, MirkStyleId? mirkStyleId})>[];
...
MirkStyle _buildStyle(MirkStyleId id, String name) =>
    MirkStyle(id: id, displayName: name, config: const AtmosphericConfig(), createdAtUtc: DateTime.utc(2026, 4, 25), createdAtOffsetMinutes: 0);

({MirkStyleSessionController controller, _FakeSessionStore sessionStore, FakeMirkStyleStore styleStore, int Function() invalidateCount}) _buildController({
  MirkStyleId? initialMirkStyleId,
}) {
  final sessionStore = _FakeSessionStore(<SessionId, Session>{_testSessionId: _buildSession(mirkStyleId: initialMirkStyleId)});
  final styleStore = FakeMirkStyleStore();
  styleStore.rows.addAll([_buildStyle(_styleAtmospheric, 'Atmospheric'), _buildStyle(_styleSolid, 'Solid')]);
  var invalidateCount = 0;
  final controller = MirkStyleSessionController(sessionStore: sessionStore, styleStore: styleStore, invalidateRenderer: () => invalidateCount++);
  return (controller: controller, sessionStore: sessionStore, styleStore: styleStore, invalidateCount: () => invalidateCount);
}

void main() {
  group('09-06 — MirkStyleSessionController (MIRK-07)', () {
    test('happy path — persists mirk_style_id + invalidates renderer once', () async {
      final ctx = _buildController();
      await ctx.controller.select(sessionId: _testSessionId, styleId: _styleSolid);
      ...
      expect(ctx.sessionStore.updateMirkStyleCalls.first, (sessionId: _testSessionId, mirkStyleId: _styleSolid));
    ...
    test('same-style reselect — no write, no invalidate', () async {
      ...
      await ctx.controller.select(sessionId: _testSessionId, styleId: _styleAtmospheric);

      expect(ctx.sessionStore.updateMirkStyleCalls, isEmpty, reason: 'no DB write on same-style reselect');
      expect(ctx.invalidateCount(), 0, reason: 'no renderer invalidation on same-style reselect');
    });

    test('unknown styleId — throws MirkStyleNotFoundException, no side effects', () async {
      final ctx = _buildController();
      await expectLater(ctx.controller.select(sessionId: _testSessionId, styleId: _styleUnknown), throwsA(isA<MirkStyleNotFoundException>()));

      expect(ctx.sessionStore.updateMirkStyleCalls, isEmpty);
      expect(ctx.invalidateCount(), 0);
    });

    test('missing session — throws NoActiveSessionException, no side effects', () async {
      final ctx = _buildController();
      // Drop the seeded session row to simulate a deleted session.
      ctx.sessionStore._sessionById.remove(_testSessionId);

      await expectLater(ctx.controller.select(sessionId: _testSessionId, styleId: _styleSolid), throwsA(isA<NoActiveSessionException>()));

      expect(ctx.sessionStore.updateMirkStyleCalls, isEmpty);
      expect(ctx.invalidateCount(), 0);
    });

    test('switching from null mirkStyleId to a real style persists + invalidates', () async {
      final ctx = _buildController();
