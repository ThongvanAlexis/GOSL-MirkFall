...
void main() {
  ...
  testWidgets('opacity stays at 0 while session is Idle', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          activeSessionControllerProvider.overrideWith(() => controller),
        ],
        child: const Directionality(
          textDirection: TextDirection.ltr,
          child: MirkInitialRevealFade(
  ...
  testWidgets(
    'opacity evolves 0 → 1 over 500 ms on Idle → Tracking transition',
    (tester) async {
      final controller = _MutableActiveSessionController(const Idle());
      await tester.pumpWidget(
        ProviderScope(
  ...
  testWidgets(
    'session ends → opacity resets to 0 (idempotence guard re-arms)',
    (tester) async {
  ...
  testWidgets(
    'second Tracking transition replays the fade (idempotence guard re-fires)',
    (tester) async {
      final controller = _MutableActiveSessionController(const Idle());
      await tester.pumpWidget(
        ProviderScope(
