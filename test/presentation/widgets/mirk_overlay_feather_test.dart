...
void main() {
  ...
  group('09-07 — MirkOverlay feather (MIRK-01)', () {
    testWidgets('paints via the active renderer when prerequisites resolved', (
      tester,
    ) async {
      ...
      expect(
        fakeRenderer.paintCallCount,
        greaterThan(0),
        reason: 'overlay must invoke renderer.paint at least once after mount',
      );
    ...
    testWidgets(
      'AtmosphericMirkRenderer paints without throwing when wired through the overlay',
      (tester) async {
          ...
          ProviderScope(
            overrides: [
              activeSessionControllerProvider.overrideWith(
                () => _FakeActiveSessionController(
                  Tracking(
                    sessionId: const SessionId('sess_atmospheric'),
                    startedAtUtc: DateTime.utc(2026, 4, 25),
                    fixCount: 0,
                    distanceFilterMeters: 5,
                  ),
  ...
  testWidgets('Ticker drives paint calls across multiple frames', (
    tester,
  ) async {
    final fakeRenderer = FakeMirkRenderer();
    final viewport = MirkViewportBbox(
      ...
      ProviderScope(
        overrides: [
          activeSessionControllerProvider.overrideWith(
            () => _FakeActiveSessionController(
              Tracking(
                sessionId: const SessionId('sess_ticker'),
                startedAtUtc: DateTime.utc(2026, 4, 25),
                fixCount: 0,
                distanceFilterMeters: 5,
              ),
    ...
    expect(
      lastElapsed.inMicroseconds,
      greaterThanOrEqualTo(firstElapsed.inMicroseconds),
      reason: 'sessionElapsed monotonically increases with the Ticker',
    );
  });
}
