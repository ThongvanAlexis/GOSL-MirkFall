...
Projet Flutter sous licence **GOSL v1.0** (Good Old Software License). Ces règles ne sont pas négociables.

## Licence du projet

Ce projet est distribué sous **GOSL v1.0**. La licence interdit explicitement la monétisation, la publicité, la télémétrie non-consentie, et les mécanismes d'abonnement dans toute redistribution ou travail dérivé.

Toute dépendance ajoutée doit être compatible avec ces contraintes.

## projet en lui meme

Application mobile "MirkFall" qui applique un "brouillard de guerre" sur la carte du monde qui se revele a mesure que l'utilisateur se deplace
...
1. Build et test sous Windows : `flutter run` (Android emulator / device), `flutter run -d windows` (desktop)
2. Tests unitaires et widgets : doivent passer avant tout commit
3. `flutter analyze` : zéro warning toléré
...
1. Tu audites le code de ton côté avec des sub-agents
2. J'audite de mon côté avec mon IDE
3. On partage nos findings et on décide quoi fixer
...
**Nommage des phases** (important pour le parsing) : pas de `.5`, utiliser des numéros séquentiels.

Exemple pour 3 phases de code :
- Phase 01 Code
...
Si un ajout de dépendance, d'appel réseau, ou de fonctionnalité **viole une des règles ci-dessus**, refuser et proposer une alternative. Ne pas ajouter d'analytics, de crash reporting automatique, ou de SDK tiers sans audit explicite documenté, même si la dépendance est "standard" dans l'industrie.

La philosophie du projet : logiciel utile, non-bloaté, respectueux de l'utilisateur. Tout ajout doit être cohérent avec ça.

Si quelqu'un ouvre MirkFall, va dans "À propos" ou "Legal", il devrait voir quelque chose comme "MirkFall is distributed under GOSL v1.0" avec idéalement un lien vers le texte complet.

chaque fichier source dois contenir ce hearder : 
