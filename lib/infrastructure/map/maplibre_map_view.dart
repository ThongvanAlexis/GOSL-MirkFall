...
Map<String, dynamic> _pointFeatureCollection({
...
class MapLibreMapViewWidget extends StatefulWidget {
  const MapLibreMapViewWidget({
    ...
    this.initialCountry,
    this.initialCamera = const CameraLatLngZoom(
      latitude: 0,
      longitude: 0,
      zoom: 2,
    ),
...
class _MapLibreMapViewWidgetState extends State<MapLibreMapViewWidget> {
  ...
  Future<void> _prefetchStyle() async {
    try {
      final String styleJson = await widget.styleRewriter
          .rewriteStyleForCountry(widget.initialCountry);
      if (!mounted) return;
      setState(() {
    ...
    return KeyedSubtree(
      ...
      child: MapLibreMap(
        ...
        onStyleLoadedCallback: () {
          ...
          if (c == null) return;
          final _MapLibreMapViewAdapter adapter = _adapter ??=
              _MapLibreMapViewAdapter(
                controller: c,
...
class _UserLocationPuckState {
  ...
  _UserLocationPuckState markLayerWiped() =>
      _UserLocationPuckState(lastFix: lastFix);

  /// After a successful `addGeoJsonSource` + `addCircleLayer` pair.
  _UserLocationPuckState markLayerInstalled() =>
      _UserLocationPuckState(layerInstalled: true, lastFix: lastFix);
  ...
  _UserLocationPuckState withFix(Fix? fix) =>
      _UserLocationPuckState(layerInstalled: layerInstalled, lastFix: fix);
}
...
class _MapLibreMapViewAdapter implements MapView {
  _MapLibreMapViewAdapter({
    ...
    _cameraListener = () {
      ...
      if (cp == null) return;
      final ({double latitude, double longitude, double zoom}) event = (
  ...
  Future<void> showMap(CountryCode? country) async {
    ...
    final CameraPosition preserved =
        prev ?? const CameraPosition(target: LatLng(0, 0), zoom: 2);
    _log.info(
    ...
    final String rewritten = await _styleRewriter.rewriteStyleForCountry(
      country,
    );
    ...
    _log.info(
      'showMap(${country?.value ?? 'world'}): camera re-applied after setStyle',
    );
    ...
    if (restore != null) {
      _log.info(
        'showMap(${country?.value ?? 'world'}): re-applying user-location puck',
      );
  ...
  Future<void> setTheme(MapTheme theme) async {
    ...
    _log.fine(
      'setTheme(${theme.toJsonString()}) — Phase 07 no-op (single theme shipped)',
    );
  }
  ...
  Future<void> setUserLocation(Fix? fix) async {
    ...
    final Map<String, dynamic> geojson = _pointFeatureCollection(
    ...
    if (!_puckState.layerInstalled) {
      ...
      _log.info(
        'setUserLocation: puck source+layer INSTALLED at (${fix.latitude}, ${fix.longitude})',
      );
    } else {
      ...
      _log.fine(
        'setUserLocation: puck UPDATED at (${fix.latitude}, ${fix.longitude})',
      );
    }
  ...
  Future<MirkViewportBbox> queryViewportBounds() async {
    if (!_aliveOrLog('queryViewportBounds')) {
      // Match the [queryViewport] convention — return a benign 0-bbox
      // rather than throwing on a disposed adapter. Callers (Phase 09
      // mapViewportProvider) treat the next viewportUpdates emission as
      // the recovery path.
  ...
  Future<void> markVisited(
    ...
    _log.fine(
      'markVisited(${polygon.length} vertices) — Phase 07 stub, no visual change',
    );
  }
  ...
  Future<void> addPointOfInterest({
    ...
    if (existing != null) {
      await _controller.updateSymbol(
        existing,
        SymbolOptions(geometry: LatLng(latitude, longitude), iconImage: iconId),
      );
      return;
    }
    final Symbol s = await _controller.addSymbol(
      SymbolOptions(geometry: LatLng(latitude, longitude), iconImage: iconId),
    );
    _poiSymbols[id] = s;
  }
