...
class JsonMigrator {
  JsonMigrator(List<JsonMigration> migrations) : _migrations = List<JsonMigration>.unmodifiable(migrations);
  ...
  Map<String, Object?> migrate({required int fromVersion, required int toVersion, required Map<String, Object?> payload}) {
    if (toVersion < fromVersion) {
      throw MigrationFailureException(reason: 'downgrade not supported (from=$fromVersion, to=$toVersion)');
    }
    var current = payload;
    var v = fromVersion;
    while (v < toVersion) {
      ...
      if (candidates.isEmpty) {
        throw MigrationFailureException(reason: 'no migrator registered for v$v -> v${v + 1}');
      }
      if (candidates.length > 1) {
        throw MigrationFailureException(reason: 'multiple migrators registered for v$v -> v${v + 1}');
      }
